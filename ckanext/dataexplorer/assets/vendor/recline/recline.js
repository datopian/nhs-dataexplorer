...
(function(my) {
  ...
  var Deferred =
    (typeof jQuery !== "undefined" && jQuery.Deferred) || _.Deferred;
  ...
  var _wrapInTimeout = function(ourFunction) {
    ...
    var timer = setTimeout(function() {
      dfd.reject({
        message:
          "Request Error: Backend did not respond after " +
          my.timeout / 1000 +
          " seconds",
      });
    }, my.timeout);
  ...
  my.Store = function(records, fields) {
    ...
    this._applyFilters = function(results, queryObj) {
      ...
      function range(record, filter) {
        var fromnull =
          _.isUndefined(filter.from) ||
          filter.from === null ||
          filter.from === "";
        var tonull =
          _.isUndefined(filter.to) || filter.to === null || filter.to === "";
        var parse = getDataParser(filter);
        var value = parse(record[filter.field]);
    ...
    this.computeFacets = function(records, queryObj) {
      ...
      _.each(queryObj.facets, function(query, facetId) {
        ...
        facetResults[facetId] = new recline.Model.Facet({
          id: facetId,
        }).toJSON();
...
(function(my) {
  ...
  my.Dataset = Backbone.Model.extend({
    ...
    initialize: function() {
      ...
      this.queryState.bind("change facet:add", function() {
        self.query(); // Re-enabled: call query() on filter changes
      });
      ...
      this._handleResult =
        this.backend != null && _.has(this.backend, "handleQueryResult")
          ? this.backend.handleQueryResult
          : this._handleQueryResult;
    ...
    fetch: function() {
      ...
      function handleResults(results) {
        ...
        if (results.useMemoryStore) {
          self._store = new recline.Backend.Memory.Store(
            out.records,
            out.fields
          );
        }
    ...
    _normalizeRecordsAndFields: function(records, fields) {
      ...
      if (
        fields &&
        fields.length > 0 &&
        (fields[0] === null || typeof fields[0] != "object")
      ) {
    ...
    getFieldsSummary: function() {
      ...
      var dfd = new Deferred();
      this._store
        .query(query.toJSON(), this.toJSON())
        .done(function(queryResult) {
          if (queryResult.facets) {
  ...
  my.Field = Backbone.Model.extend({
    ...
    initialize: function(data, options) {
      ...
      if ("0" in data) {
        throw new Error(
          "Looks like you did not pass a proper hash with id to Field constructor"
        );
      }
      ...
      if (this.attributes.type.toLowerCase() in this._typeMap) {
        this.attributes.type = this._typeMap[
          this.attributes.type.toLowerCase()
        ];
      }
      if (options) {
  ...
  my.Flot = Backbone.View.extend({
    ...
    <div class="recline-flot"> \
      ...
      "</p> \
          <p>" +
      ckan.i18n._(
        "Please tell us by <strong>using the menu on the right</strong> and a graph will automatically appear."
      ) +
      "</p> \
        </div> \
      </div> \
...
",
    ...
    redraw: function() {
      ...
      if (this.state.get("group") && this.state.get("series")) {
        ...
        var options = this.getGraphOptions(
          this.state.attributes.graphType,
    ...
    _toolTip: function(event, pos, item) {
      if (item) {
        if (
          ...
          $('<div id="recline-flot-tooltip">' + content + "</div>")
            .css({
              top: yLocation,
              left: xLocation,
            })
    ...
    _groupFieldIsDateTime: function() {
      ...
      var isDateTime =
        xtype === "date" || xtype === "date-time" || xtype === "time";
      return isDateTime;
    },
    ...
    createSeries: function() {
      ...
      _.each(this.state.attributes.series, function(field) {
        ...
        if (isDateTime) {
          ...
          var raw = _.map(self.model.records.models, function(doc, index) {
            return doc.getFieldValueUnrendered(xfield);
          });

          if (
            _.all(raw, function(x) {
    ...
    <form class="form-stacked"> \
      <div class="clearfix"> \
        <div class="form-group"> \
          <label>' +
      ckan.i18n._("Graph Type") +
      '</label> \
          <div class="input editor-type"> \
            <select class="form-control"> \
              <option value="lines-and-points">' +
      ckan.i18n._("Lines and Points") +
      '</option> \
              <option value="lines">' +
      ckan.i18n._("Lines") +
      '</option> \
      ...
      '</option> \
        ...
        <div class="form-group"> \
          <label>' +
      ckan.i18n._("Group Column (Axis 1)") +
      '</label> \
          <div class="input editor-group"> \
            <select class="form-control"> \
              <option value="">' +
      ckan.i18n._("Please choose ...") +
      '</option> \
                {{#fields}} \
      ...
      <div class="editor-buttons"> \
        <button class="btn btn-default editor-add">' +
      ckan.i18n._("Add Series") +
      '</button> \
      </div> \
...
(function($, my) {
  ...
  my.Grid = Backbone.View.extend({
    ...
    showColumn: function(e) {
      var hiddenFields = _.without(
        this.state.get("hiddenFields"),
  ...
  ',
    ...
    render: function() {
      var self = this;
      this.fields = new recline.Model.FieldList(
        this.model.fields.filter(function(field) {
          return _.indexOf(self.state.get("hiddenFields"), field.id) == -1;
        })
      );

      this.scrollbarDimensions =
        this.scrollbarDimensions || this._scrollbarSize(); // skip measurement if already have dimensions
      var numFields = this.fields.length;
      // compute field widths (-20 for first menu col + 10px for padding on each col and finally 16px for the scrollbar)
      var fullWidth =
        self.$el.width() - 20 - 10 * numFields - this.scrollbarDimensions.width;
      ...
      this.$el
        .find(".recline-grid")
        .toggleClass("no-hidden", self.state.get("hiddenFields").length === 0);
    ...
    _scrollbarSize: function() {
      ...
      var dim = {
        width: $c.width() - $c[0].clientWidth + 1,
  ...
  ',
    ...
    onEditClick: function(e) {
      ...
      if (editing.length > 0) {
        editing
          .parents(".data-table-cell-value")
          .html(editing.text())
          .siblings(".data-table-cell-edit")
          .removeClass("hidden");
      }
      $(e.target).addClass("hidden");
      var cell = $(e.target).siblings(".data-table-cell-value");
      cell.data("previousContents", cell.text());
      var templated = Mustache.render(this.cellEditorTemplate, {
        value: cell.text(),
      });
      cell.html(templated);
    },
    ...
    onEditorOK: function(e) {
      ...
      var newValue = cell
        .parents(".data-table-cell-editor")
    ...
    onEditorCancel: function(e) {
      ...
      cell
        .html(cell.data("previousContents"))
        .siblings(".data-table-cell-edit")
        .removeClass("hidden");
    },
  });
    ...
    <form class="form-stacked"> \
      <div class="clearfix"> \
        <div class="editor-field-type"> \
            <label class="radio"> \
              ...
              ' + ckan.i18n._('Latitude / Longitude fields') + '</label> \
            <label class="radio"> \
              <input type="radio" id="editor-field-type-geom" name="editor-field-type" value="geom" /> \
              ' + ckan.i18n._('GeoJSON field') + '</label> \
        </div> \
      ...
      <div class="editor-options" > \
        <label class="checkbox"> \
          ...
          ' + ckan.i18n._('Auto zoom to features') + '</label> \
        <label class="checkbox"> \
          <input type="checkbox" id="editor-cluster" value="cluster"/> \
          ' + ckan.i18n._('Cluster markers') + '</label> \
      </div> \
  ...
  <div class="recline-data-explorer"> \
    ...
    <div class="header clearfix"> \
      ...
      <div class="recline-results-info"> \
        <span class="doc-count">{{recordCount}}</span> ' +
      ckan.i18n._("records") +
      '\
      </div> \
  ...
  ',
    ...
    initialize: function(options) {
      ...
      this.listenTo(this.model, "query:done", function() {
        ...
        self.$el
          .find(".doc-count")
          .text(self.model.recordCount || ckan.i18n._("Unknown"));
        var from = parseInt(this.model.queryState.get("from"));
        var to = Math.min(
          from + this.model.queryState.get("size"),
          this.model.recordCount
        );
    ...
    updateNav: function(pageName) {
      ...
      var $el = this.$el.find(
        '.navigation button[data-view="' + pageName + '"]'
      );
      $el.addClass("active");
    ...
    notify: function(flash) {
      ...
      _templated = $(_templated).appendTo(
        $(".recline-data-explorer .alert-messages")
      );
      if (!flash.persist) {
        setTimeout(function() {
    ...
    if (state.backend === "memory") {
      datasetInfo = {
        ...
        records: [
          {
            stub:
              "this is a stub dataset because we do not restore memory datasets",
          },
        ],
      };
    } else {
...
(function($, my) {
  ...
  my.SlickGrid = Backbone.View.extend({
    initialize: function(modelEtc) {
      ...
      this.templates = {
        deleterow:
          '<button href="#" class="recline-row-delete btn btn-default" title="Delete row">X</button>',
      };
    ...
    render: function() {
      ...
      _.each(this.model.fields.toJSON(), function(field) {
        ...
        var width = parseInt(
          Math.max(column.minWidth, fullWidth / numFields),
          10
        );
        column.width = width;
      ...
      if (
        this.state.get("columnsOrder") &&
        this.state.get("columnsOrder").length > 0
      ) {
        visibleColumns = visibleColumns.sort(function(a, b) {
          return _.indexOf(self.state.get("columnsOrder"), a.id) >
            _.indexOf(self.state.get("columnsOrder"), b.id)
            ? 1
            : -1;
        });
    ...
    */
      ...
      if (
        this.state.get("gridOptions") &&
        this.state.get("gridOptions").enableReOrderRow
      ) {
      ...
      this._slickHandler.subscribe(this.grid.onClick, function(e, args) {
        ...
        if (
          args.cell == cell &&
          self.state.get("gridOptions").enabledDelRow == true
        ) {
  ...
  my.GridControl = Backbone.View.extend({
    ...
    template:
      '<h1><button href="#" class="recline-row-add btn btn-default">Add row</button></h1>',
...
(function($) {
  function SlickColumnPicker(columns, grid, model, options) {
    ...
    function init() {
      ...
      $menu = $(
        '<ul class="dropdown-menu slick-contextmenu" style="display:none;position:absolute;z-index:20;" />'
    ...
    function handleHeaderContextMenu(e, args) {
      ...
      for (var i = 0; i < columns.length; i++) {
        ...
        $input = $('<input type="checkbox" />')
          ...
          .attr("id", "slick-column-vis-" + columns[i].id);
      ...
      $input = $('<input type="checkbox" />')
        ...
        .attr("id", "slick-option-autoresize");
      $input.appendTo($li);
      $("<label />")
    ...
    function updateColumn(e) {
      ...
      if (
        ($(e.target).is("li") && !$(e.target).hasClass("divider")) ||
        $(e.target).is("input")
      ) {
        if ($(e.target).is("li")) {
          checkbox = $(e.target).find("input").first();
  ...
  ',
    ...
    _checkField: function(possibleFieldNames) {
      ...
      for (var i = 0; i < possibleFieldNames.length; i++) {
        for (var j = 0; j < modelFieldNames.length; j++) {
          if (
            modelFieldNames[j].toLowerCase() ==
            possibleFieldNames[i].toLowerCase()
          )
            return modelFieldNames[j];
        }
      }
...
(function($, my) {
  ...
  ',
    ...
    onFacetFilter: function(e) {
      ...
      this.model.queryState.addFilter({
        type: "term",
        field: fieldId,
        term: value,
      });
  ...
  ',
    ...
    initialize: function() {
      ...
      this.listenTo(
        ...
        "change change:filters:new-blank",
        this.render
      );
      this.render();
    },
  ...
  my.Pager = Backbone.View.extend({
    ...
    <div class="pagination"> \
      <ul class="pagination"> \
        ...
        <li class="page-range"><a><label for="from">From</label><input id="from" name="from" aria-label="From" type="text" value="{{from}}" /> &ndash; <label for="to">To</label><input id="to" name="to" aria-label="To" type="text" value="{{to}}" /> </a></li> \
  ...
  ',
    ...
    onFormSubmit: function(e) {
      ...
      if (this.model.queryState.get("from") != formFrom) {
        ...
        this.model.queryState.set({
          from: Math.min(maxRecord, Math.max(formFrom, 0)),
        });
      } else if (this.model.queryState.get("to") != formTo) {
        // change to; update size
        var to = Math.min(maxRecord, Math.max(formTo, 0));
        this.model.queryState.set({
          size: Math.min(maxRecord + 1, Math.max(to - formFrom + 1, 1)),
        });
      }
    },
    ...
    <form action="" method="GET" class="form-inline" role="form"> \
      <div class="form-group"> \
        <div class="input-group text-query"> \
          ...
          <input class="form-control search-query" type="text" id="q" name="q" value="{{q}}" placeholder=' +
      ckan.i18n._("Search data ...") +
      '> \
        </div> \
      </div> \
...
(function($, my) {
  ...
  my.ValueFilter = Backbone.View.extend({
    ...
    <div class="filters"> \
      ...
      ckan.i18n._("Filters") +
      '</h3> \
      <button class="btn js-add-filter add-filter">' +
      ckan.i18n._("Add filter") +
      '</button> \
      <form class="form-stacked js-add" style="display: none;"> \
        <fieldset> \
          <label>' +
      ...
      <form class="form-stacked js-edit"> \
        ...
        {{/filters}} \
        {{#filters.length}} \
        <button type="submit" class="btn update-filter">' +
      ckan.i18n._("Update") +
      "</button> \
        {{/filters.length}} \
      </form> \
  ...
  my.Extractor = Backbone.View.extend({
  ...
  ",
    ...
    extractFile: function(self, sql_query, format, query = {}) {
      ...
      fetch(endpoint)
        .then(async (resp) => {
          ...
          if (resource.success) {
            if (resource.result.hasOwnProperty("gc_urls")) {
    ...
    jsQueryToSQL: function(query_obj, model_fields) {
      ...
      if ("distinct" in query_obj) {
        distinct = "DISTINCT";
      } else {
        distinct = "";
      }

      if ("fields" in query_obj && query_obj["fields"].length > 0) {
        let string_fields = "";
        query_obj["fields"].forEach((field) => {
          string_fields += String(field);
          string_fields += ",";
        });

        query = `SELECT ${distinct} ${string_fields}`;
      } else {
        query = "SELECT * ";
      }
      ...
      if ("sort" in query_obj) {
        query += ` ORDER BY ${query_obj["sort"]} `;
      }
      query += ` LIMIT ${query_obj["limit"]}`;

      if ("offset" in query_obj) {
        query += ` OFFSET ${query_obj["offset"]}`;
      }
      return query;
    },
    where_clauses: function(fields, query_obj, model_fields) {
      ...
      if (Object.keys(filters).length != 0) {
        ...
        where_str += where_filters.slice(0, -4);
      }
