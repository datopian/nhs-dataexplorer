this.ckan.module('recline_view',function(jQuery,_){return{options:{site_url:"",controlsClassName:"controls"},initialize:function(){function loadScript(url,callback)
...
var scriptLoaded=function(){jQuery.proxyAll(this,/_on/);this.options.resource=JSON.parse(this.options.resource);this.options.resourceView=JSON.parse(this.options.resourceView);this.el.ready(this._onReady);L.Icon.Default.imagePath=this.options.site_url+'vendor/leaflet/0.7.7/images';}.bind(this);loadScript('/vendor/recline/recline.js',scriptLoaded)},_onReady:function(){var resourceData=this.options.resource,resourceView=this.options.resourceView;this.loadView(resourceData,resourceView);},loadView:function(resourceData,reclineView){var self=this;function showError(msg){msg=msg||self._('error loading view').fetch();window.parent.ckan.pubsub.publish('data-viewer-error',msg);}
...
var errorMsg,dataset,map_config;resourceData.backend='ckan';resourceData.endpoint=jQuery('body').data('site-root')+'api';
...
view=new recline.View.Map(this._reclineMapViewOptions(dataset,this.options.map_config));}else if(reclineView.view_type==="recline_view"||reclineView.view_type==="dataexplorer"){view=this._newDataExplorer(dataset,this.options.map_config,reclineView);}else{view=new recline.View.SlickGrid({model:dataset});controls=[new recline.View.Pager({model:view.model}),new recline.View.RecordCount({model:dataset}),new recline.View.QueryEditor({model:view.model.queryState})];}
if(reclineView.view_type!=='recline_view'&&reclineView.view_type!=='dataexplorer'){var newElements=jQuery('<div />');this._renderControls(newElements,controls,this.options.controlsClassName);newElements.append(view.el);jQuery(this.el).html(newElements);view.visible=true;view.render();}
